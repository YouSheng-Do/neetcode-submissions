...
public:
    TreeNode* buildTree(vector<int>& preorder, vector<int>& inorder) {
        // preorder has the root at first
        ...
        int index = 0;
        return buildNode(preorder, 0, sz - 1, index, nodeToPos);
    }

    TreeNode* buildNode(vector<int>& preorder, int l, int r, int& i, unordered_map<int, int>& nodeToPos) {
        ...
        root->left = buildNode(preorder, l, pos - 1, i, nodeToPos);

        root->right = buildNode(preorder, pos + 1, r, i, nodeToPos);
