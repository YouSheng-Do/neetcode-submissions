...
public:
    ...
    bool z_function(vector<string>& text, vector<string>& pattern) {
        ...
        // use z-box [l, r] represent that z[l....r] is the same with z[0....r-l]
        // O(m + n)
        ...
        for (int i = 1; i < n; ++i) {
            // [l....i.....r]
            // => s[l...i] is the same with prefix s[0...i-l]
            // we want to know z[i] and because we know s[i....r] is equal to s[i-l...r-l], we can reuse z[i - l]
            // z[i - l] represent the longest start from i-l match to prefix
            // however, z[i - l] may exceed r boundary.
            // so we use min()
            if (i <= r) {
                z[i] = min(z[i - l], r - i + 1);
            }
