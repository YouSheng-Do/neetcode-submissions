...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if ((nums1.size() + nums2.size()) % 2) {
            while (l <= r) {
                int m = l + (r - l) / 2;
                int new_index = m + binarySearch1(nums1[m], nums2);
                if (new_index == median) return nums1[m];
            ...
            while (l <= r) {
                int m = l + (r - l) / 2;
                int new_index = m + binarySearch2(nums2[m], nums1);
                if (new_index == median) return nums2[m];
                if (new_index > median) r = m - 1;
                else l = m + 1;
            }
        }
            ...
            while (l <= r) {
                int m = l + (r - l) / 2;
                int pos = binarySearch1(nums1[m], nums2);
                int new_index = m + pos;
            ...
            while (l <= r) {
                int m = l + (r - l) / 2;
                int pos = binarySearch2(nums2[m], nums1);
                int new_index = m + pos;
                if (new_index == median - 1) {
                    int nums1_next = (pos < nums1.size()) ? nums1[pos] : INT_MAX;
                    int nums2_next = (m < nums2.size() - 1) ? nums2[m + 1] : INT_MAX;
                    return (double) (nums2[m] + min(nums1_next, nums2_next)) / 2.0;
                }
                if (new_index == median) {
                    int nums1_prev = (pos > 0) ? nums1[pos - 1] : INT_MIN;
                    int nums2_prev = (m > 0) ? nums2[m - 1] : INT_MIN;
                    return (double) (nums2[m] + max(nums1_prev, nums2_prev)) / 2.0;
                }
                if (new_index > median) r = m - 1;
                else l = m + 1;
            }
    ...
    int binarySearch1(int target, vector<int>& nums) {
    ...
    int binarySearch2(int target, vector<int>& nums) {
        // return the insertion pos
        int l = -1, r = nums.size();
        // -inf, nums[0], nums[1], .........nums[n - 1], inf
        // T,............................................F
