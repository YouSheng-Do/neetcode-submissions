...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if ((nums1.size() + nums2.size()) % 2) {
            double ans = findMedianForOdd(nums1, nums2, true);
            if (ans != -DBL_MAX) return ans;
            return findMedianForOdd(nums2, nums1, false);
        }

        double ans = findMedianForEven(nums1, nums2, true);
        if (ans != -DBL_MAX) return ans;
        return findMedianForEven(nums2, nums1, false);
    }
    ...
    double findMedianForOdd(vector<int>& nums1, vector<int>& nums2, bool equal) {
        int l = 0, r = nums1.size() - 1;
        int median = (nums1.size() + nums2.size()) / 2;

        while (l <= r) {
            int m = l + (r - l) / 2;
            int new_index = m + binarySearch(nums1[m], nums2, equal);
    ...
    double findMedianForEven(vector<int>& nums1, vector<int>& nums2, bool equal) {
        int l = 0, r = nums1.size() - 1;
        int median = (nums1.size() + nums2.size()) / 2;

        while (l <= r) {
            int m = l + (r - l) / 2;
            int pos = binarySearch(nums1[m], nums2, equal);
            int new_index = m + pos;
